...
export default function OrganizerEvents() {
  ...
  const [activeTab, setActiveTab] = useState(
    searchParams.get('tab') === 'ended' ? 'ended' : null,
  );
  ...
  async function load(tab) {
    ...
    try {
      if (tab === 'ended') {
        const data = await getMyEvents('approved');
        const now = new Date().toISOString();
        setEvents(data.filter((e) => e.ends_at < now));
      } else {
        const data = await getMyEvents(tab);
        setEvents(data);
      }
    } catch (e) {
      setError(e.message);
      ...
      <main className='mx-auto max-w-6xl px-4 py-10'>
        ...
        >
          ...
          <Link
            ...
            className='inline-flex items-center gap-2 rounded-2xl bg-[#272F54] px-5 py-2 text-sm font-semibold text-white shadow transition hover:bg-[#1e2544] active:scale-95'
          >
            ...
            <AnimatePresence>
              <div className='space-y-4'>
                {events.map((event, i) => {
                  const now = new Date().toISOString();
                  const isEnded =
                    event.status === 'approved' && event.ends_at < now;
                  const sm = isEnded
                    ? { label: 'Încheiat', color: 'bg-slate-100 text-slate-500' }
                    : (STATUS_META[event.status] ?? STATUS_META.draft);
                  return (
                    <motion.div
                      key={event.id}
                    ...
                    >
                      {event.cover_image_url && (
                        <img
                          ...
                          style={{
                            objectPosition:
                              event.cover_image_position || '50% 50%',
                          }}
                        />
                      )}
                      ...
                      <div className='flex flex-shrink-0 gap-2'>
                        {(event.status === 'draft' ||
                          ...
                          <button
                            onClick={() =>
                              navigate(
                                `/organizator/eveniment/${event.id}/editeaza`,
                              )
                            }
                        ...
                        {event.status === 'draft' && (
                          ...
                          >
                            <Send className='h-3.5 w-3.5' /> Trimite spre
                            validare
                          </button>
                        )}
                        <button
                          onClick={() =>
                            navigate(
                              `/organizator/eveniment/${event.id}/materiale`,
                            )
                          }
                          className='inline-flex items-center gap-1 rounded-xl border border-[#272F54]/20 bg-white px-3 py-2 text-xs font-semibold text-[#272F54] transition hover:border-[#272F54]/50'
                        >
                        ...
                        {event.participation_type !== 'free' && (
                          <button
                            onClick={() =>
                              navigate(
                                `/organizator/eveniment/${event.id}/participanti`,
                              )
                            }
                            className='inline-flex items-center gap-1 rounded-xl border border-[#272F54]/20 bg-white px-3 py-2 text-xs font-semibold text-[#272F54] transition hover:border-[#272F54]/50'
                          >
                        ...
                        <button
                          onClick={() =>
                            navigate(
                              `/organizator/eveniment/${event.id}/statistici`,
                            )
                          }
                          className='inline-flex items-center gap-1 rounded-xl border border-[#272F54]/20 bg-white px-3 py-2 text-xs font-semibold text-[#272F54] transition hover:border-[#272F54]/50'
                        >
