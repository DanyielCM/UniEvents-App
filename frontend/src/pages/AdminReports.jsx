...
export default function AdminReports() {
  ...
  const monthlyChartData =
    monthly?.months.filter((m) => m.events_count > 0) || [];
  const eventsInYear =
    monthly?.months.reduce((s, m) => s + m.events_count, 0) || 0;
    ...
    <div className='min-h-screen bg-gradient-hero'>
      ...
      <main className='mx-auto max-w-6xl px-4 py-10'>
        ...
        {loading ? (
          ...
          <div className='mt-6 rounded-2xl bg-[#FF8383]/20 p-4 text-sm text-rose-800'>
            {error}
          </div>
        ) : (
          <>
            ...
            <div className='mt-4 grid gap-4 sm:grid-cols-3'>
              <div className='rounded-2xl border border-white/60 bg-white/80 p-4 text-center backdrop-blur'>
                <p className='text-xs font-semibold uppercase tracking-wider text-[#272F54]/50'>
                  Aprobate
                </p>
                <p className='font-display text-2xl font-bold text-[#272F54] mt-1'>
                  {overview.approved_events}
                </p>
              </div>
              <div className='rounded-2xl border border-white/60 bg-white/80 p-4 text-center backdrop-blur'>
                <p className='text-xs font-semibold uppercase tracking-wider text-[#272F54]/50'>
            ...
            <div className='mt-6 rounded-3xl border border-white/60 bg-white/85 p-5 backdrop-blur shadow-[0_8px_24px_-12px_rgba(39,47,84,0.2)]'>
              <div className='flex items-center justify-between mb-4 flex-wrap gap-3'>
                <div>
                  ...
                  <p className='text-xs text-slate-500'>
                    {eventsInYear} evenimente în total
                  </p>
                </div>
                    ...
                    <button
                      ...
                      className={`rounded-xl px-3 py-1.5 text-sm font-semibold transition ${
                        y === year
                          ...
                          : 'border border-[#272F54]/20 text-[#272F54]/60 hover:text-[#272F54]'
                      }`}
                    >
                      {y}
                    </button>
              ...
              {monthlyChartData.length > 0 ? (
                  ...
                  >
                    ...
                    <Tooltip
                      formatter={(value, name) => [
                        value,
                        name === 'events_count' ? 'evenimente' : 'înscrieri',
                      ]}
                    />
                    <Bar
                ...
                <p className='text-center text-sm text-[#272F54]/40 py-10'>
                  Niciun eveniment aprobat în {year}.
                </p>
              )}
            </div>
