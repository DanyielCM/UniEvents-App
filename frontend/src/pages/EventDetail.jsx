...
export default function EventDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      user?.role === 'student'
        ? getMyRegistrationForEvent(id).catch(() => null)
  ...
  if (error || !event) {
    return (
      <div className='flex min-h-screen flex-col items-center justify-center gap-4 bg-gradient-hero'>
        <p className='text-lg font-semibold text-[#272F54]/60'>
          ...
          <div className='grid gap-8 lg:grid-cols-3'>
            ...
            <div className='lg:col-span-2'>
              ...
              <div className='mt-5 flex flex-wrap items-center gap-2'>
                {event.category && (
                  <span
                    className='inline-flex items-center justify-center rounded-full px-3 py-1 text-xs font-semibold text-white uppercase tracking-wider'
                    style={{ backgroundColor: color }}
                  >
                    {event.category.name}
                  </span>
                )}
                <span className='inline-flex items-center justify-center rounded-full border border-[#272F54]/15 bg-white/80 px-3 py-1 text-xs font-semibold text-[#272F54]'>
                  ...
                  <div className='space-y-2'>
                    {materials.map((mat) => (
                      <button
                        key={mat.id}
                        onClick={() =>
                          downloadFile(mat.download_url, mat.original_name)
                        }
              ...
              {/* Feedback form — only for students who attended this event */}
              {user?.role === 'student' &&
                event &&
              ...
              <div className='rounded-3xl border border-white/60 bg-white/85 p-5 shadow-[0_10px_30px_-20px_rgba(39,47,84,0.2)] backdrop-blur space-y-2'>
                        ...
                        <div className='space-y-1'>
                          <div className='flex justify-between text-xs text-[#272F54]/60'>
                            ...
                            <span className='font-semibold'>
                              {Math.max(
                                0,
                                regInfo.capacity - regInfo.confirmed_count,
                              )}
                              /{regInfo.capacity}
                            </span>
                          </div>
                        ...
                        <div className='space-y-2'>
                          ...
                          {myReg.status === 'confirmed' && (
                            <button
                              ...
                              className='flex w-full items-center justify-center gap-2 rounded-2xl border border-[#FF8383]/40 bg-[#FF8383]/10 px-4 py-2 text-sm font-semibold text-rose-700 disabled:opacity-50'
                            >
                              {regBusy ? (
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              setMyReg(reg);
                              setRegInfo((prev) =>
                                prev && reg.status === 'confirmed'
                                  ? {
                                      ...prev,
                                      confirmed_count: prev.confirmed_count + 1,
                                      is_full:
                                        prev.confirmed_count + 1 >=
                                        prev.capacity,
                                    }
                                  : prev,
                              );
                          ...
                          className={`flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-2 text-sm font-semibold disabled:opacity-50 transition ${
                            regInfo?.is_full
                ...
                {event.registration_link && (
                  <a
                    ...
                    target='_blank'
                    rel='noreferrer'
                    className='flex w-full items-center justify-center gap-2 rounded-2xl border border-[#272F54]/20 bg-white/80 px-4 py-2 text-sm font-semibold text-[#272F54] transition hover:border-[#272F54]/40'
                  >
              ...
              {feedbackSummary?.count > 0 && (
                <div className='rounded-3xl border border-white/60 bg-white/85 p-5 shadow-[0_10px_30px_-20px_rgba(39,47,84,0.2)] backdrop-blur'>
                  ...
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      <StarRating
                        rating={Math.round(feedbackSummary.avg_rating || 0)}
                        readOnly
                        size='sm'
                      />
                      <p className='mt-0.5 text-xs text-slate-500'>
                        {feedbackSummary.count} recenzi
                        {feedbackSummary.count === 1 ? 'e' : 'i'}
                      </p>
                    </div>
                  </div>
                </div>
