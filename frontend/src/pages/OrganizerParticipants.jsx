  ...
  async function handleExport() {
    ...
    try {
      const resp = await fetch(
        `${API_BASE}/registrations/events/${eventId}/export`,
        {
          ...
          <div className='mt-6 rounded-2xl border border-white/60 bg-white/80 p-4 backdrop-blur'>
            ...
            <div className='flex gap-2'>
              <input
                ...
                className='flex-1 rounded-xl border border-[#272F54]/15 bg-white/80 px-4 py-2 font-mono text-sm text-[#272F54] outline-none focus:border-[#83BDE5] focus:ring-2 focus:ring-[#83BDE5]/20'
              />
              <button
                disabled={!manualToken.trim() || busy === manualToken.trim()}
                onClick={() => handleCheckin(manualToken.trim())}
                className='inline-flex items-center gap-1 rounded-xl bg-[#272F54] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50 hover:bg-[#1e2544]'
              >
                {busy ? (
                ...
                {participants.map((p, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`inline-flex items-center justify-center rounded-full px-2.5 py-0.5 text-xs font-semibold flex-shrink-0 ${sm.color}`}
                      >
                        {sm.label}
                      </span>
