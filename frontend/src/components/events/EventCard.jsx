...
export default function EventCard({ event, index = 0 }) {
  ...
  return (
    ...
    >
      {/* Top accent stripe */}
      <div className='h-6 w-full flex-shrink-0' style={{ background: color }} />
      ...
      <div className='flex flex-1 flex-col gap-3 p-5'>
        ...
        <div className='flex flex-wrap items-center gap-2'>
          {event.category && (
            <span
              className='inline-flex items-center justify-center rounded-full px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wider text-white'
              style={{ backgroundColor: color }}
            >
              {event.category.name}
            </span>
          )}
          <span className='inline-flex items-center justify-center rounded-full border border-[#272F54]/10 bg-white/70 px-2.5 py-0.5 text-[11px] font-semibold text-[#272F54]/70'>
